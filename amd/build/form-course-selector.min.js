 ...
 * @since      3.1
 */
define("local_solsits/form-course-selector",["core/ajax","jquery"],(function(ajax,$){return{processResults:function(selector,data){var results=[],i=0,excludelist=String($(selector).data("exclude")).split(",");for(i=0;i<data.length;i++)-1===excludelist.indexOf(String(data[i].courseid))&&results.push({value:data[i].courseid,label:data[i].label});return results},transport:function(selector,query,success,failure){var promises,currentcourses=document.querySelector("#id_currentcourses");void 0===query&&(query="");var calls=[{methodname:"local_solsits_search_courses",args:{query:query,currentcourses:currentcourses=!!currentcourses&&currentcourses.checked}}];promises=ajax.call(calls),$.when.apply($.when,promises).done((function(data){success(data)})).fail(failure)}}}));
